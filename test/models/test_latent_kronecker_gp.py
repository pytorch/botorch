...
class TestLatentKroneckerGP(BotorchTestCase):
    def test_gp(self):
        for (
            ...
            if missing_vals:
                ...
                train_Y_valid = torch.ones(n_train * m, dtype=bool, device=self.device)
            ...
            model.to(**tkwargs)
            model.likelihood.to(**tkwargs)

            # test init
            train_Y_flat = (
                train_Y.reshape(*batch_shape, -1)[..., train_Y_valid.reshape(-1)]
                if missing_vals
                else train_Y.reshape(*batch_shape, -1)
            )
            if use_transforms:
                ...
                with self.assertRaises(ValueError):
                    octf(torch.zeros(*batch_shape, 0, m, **tkwargs))
                octf._is_trained = torch.tensor(False)
                # stdvs calculation with single observation
                octf(torch.zeros(*batch_shape, 1, m, **tkwargs))
                self.assertAllClose(
                    octf.stdvs, torch.ones(*batch_shape, 1, m, **tkwargs)
                )
                octf._is_trained = torch.tensor(False)
            ...
            train_Y_flat = (
                octf(train_Y_flat.unsqueeze(-1))[0].squeeze(-1)
                if use_transforms
                else train_Y_flat
            )
            ...
            for test_shape in (
                ...
                for sample_shape in ((), (1,), (2, 3)):
                    ...
                    base_samples = torch.randn(
                        torch.Size(sample_shape) + posterior.base_sample_shape,
                        **tkwargs,
                    )
                    with warnings.catch_warnings(), model.use_iterative_methods():
                        warnings.filterwarnings("ignore", category=NumericalWarning)
                    ...
                    if len(sample_shape) > 0:
                        ...
                        incorrect_base_samples = torch.randn(
                            torch.Size([5]) + posterior.base_sample_shape,
                            **tkwargs,
                        )
                        with self.assertRaises(RuntimeError):
                ...
                with model.use_iterative_methods():
                    pred = model(intf(test_X)) if use_transforms else model(test_X)
                ...
                pred_mean, pred_var = (
                    octf.untransform(pred_mean, pred_var)
                    if use_transforms
    ...
    def test_custom_init(self):
        ...
        for batch_shape, n_train, d, m, missing_vals, dtype in itertools.product(
            (  # batch_shape
                torch.Size([]),
                torch.Size([1]),
                torch.Size([2]),
            ...
            if missing_vals:
                ...
                train_Y_valid = train_Y_valid.reshape(n_train, m)
                train_Y[..., ~train_Y_valid] = torch.nan
                mask = train_Y_valid.reshape(n_train * m)
            else:
                train_Y_valid = None
                mask = torch.ones(n_train * m, dtype=bool, device=self.device)
            ...
            self.assertAllClose(model.mask, mask, atol=0.0)
