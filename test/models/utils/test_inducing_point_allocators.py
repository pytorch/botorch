...
from gpytorch.kernels import MaternKernel, MultitaskKernel
...
class TestExpectedImprovementQualityFunction(BotorchTestCase):
    def setUp(self):
        ...
        mll = VariationalELBO(self.previous_model.likelihood, self.previous_model.model, num_data=10)
        loss = -mll(self.previous_model.likelihood(self.previous_model(train_X)), train_y).sum()
        loss.backward()
    ...
    def test_returns_correct_shape(self):
        ...
        for maximize in [True, False]:
            quality_function = ExpectedImprovementQualityFunction(self.previous_model, maximize=maximize)
            scores = quality_function(train_X)
            self.assertEqual(scores.shape, torch.Size([15]))


# todo test raise error for multi-output
# todo test ei calc
...
class TestGreedyVarianceReduction(BotorchTestCase):
    ...
    def test_that_we_dont_get_redundant_inducing_points(self):
        ...
        # are passed
        self.assertLess(
            inducing_points_1.shape[-2], num_inducing
        )
