...
class TestNegLevy(BotorchTestCase):
    ...
    def test_single_eval_neg_levy_cuda(self):
        if torch.cuda.is_available():
            self.test_single_eval_neg_levy(cuda=True)

    def test_batch_eval_neg_levy(self, cuda=False):
        device = torch.device("cuda") if cuda else torch.device("cpu")
        for dtype in (torch.float, torch.double):
    ...
    def test_batch_eval_neg_levy_cuda(self):
        if torch.cuda.is_available():
            self.test_batch_eval_neg_levy(cuda=True)

    def test_neg_levy_global_maximum(self, cuda=False):
        device = torch.device("cuda") if cuda else torch.device("cpu")
        for dtype in (torch.float, torch.double):
            X = torch.tensor(
