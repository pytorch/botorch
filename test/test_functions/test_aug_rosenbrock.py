...
class TestNegAugRosenbrock(BotorchTestCase):
    def test_single_eval_neg_aug_rosenbrock(self):
        for dtype in (torch.float, torch.double):
            X = torch.zeros(5, device=self.device, dtype=dtype)
            res = neg_aug_rosenbrock(X)
            self.assertEqual(res.dtype, dtype)
            self.assertEqual(res.device.type, self.device.type)
            self.assertEqual(res.shape, torch.Size())

    def test_batch_eval_neg_aug_rosenbrock(self):
        for dtype in (torch.float, torch.double):
            X = torch.zeros(2, 5, device=self.device, dtype=dtype)
            res = neg_aug_rosenbrock(X)
            self.assertEqual(res.dtype, dtype)
            self.assertEqual(res.device.type, self.device.type)
            self.assertEqual(res.shape, torch.Size([2]))

    def test_neg_aug_rosenbrock_global_maximum(self):
        for dtype in (torch.float, torch.double):
            X = torch.tensor(
