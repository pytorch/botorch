...
# 
# We use the parallel ParEGO ($q$ParEGO) [1] and parallel Noisy Expected Hypervolume Improvement ($q$NEHVI) [2]  acquisition functions to optimize a synthetic C2-DTLZ2 test function with $M=2$ objectives, $V=1$ constraint, and $d=12$ parameters. The two objectives are
...
def initialize_model(train_x, train_obj, train_con):
    ...
    for i in range(train_y.shape[-1]):
        models.append(
            SingleTaskGP(train_x, train_y[..., i:i+1], outcome_transform=Standardize(m=1))
        )
...
hvs_qparego_all, hvs_qnehvi_all, hvs_random_all = [], [], []
