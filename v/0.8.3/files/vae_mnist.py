...
cnn_state_dict = torch.load(
    os.path.join(PRETRAINED_LOCATION, "mnist_cnn.pt"), map_location=device
)
...
vae_state_dict = torch.load(
    os.path.join(PRETRAINED_LOCATION, "mnist_vae.pt"), map_location=device
)
...
def gen_initial_data(n=5):
    ...
    train_obj = score_image_recognition(decode(train_x)).unsqueeze(
        -1
...
def optimize_acqf_and_get_observation(acq_func):
    ...
    new_obj = score_image_recognition(decode(new_x)).unsqueeze(
        -1
    )  # add output dimension
