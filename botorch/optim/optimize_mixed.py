...
def get_categorical_neighbors(
    ...
    Returns:
        A tensor of shape `num_neighbors x d`, denoting all unique 1-Hamming
        distance neighbors.
    """

    # Neighbors are generated by considering all possible values for each
    # categorical dimension, one at a time.
    def _get_cat_values_tensor(lb: Tensor, ub: Tensor) -> Tensor:
        r"""Get a tensor of all of the values a categorical feature may take."""
        if ub - lb + 1 <= max_num_cat_values:
            return torch.arange(
                lb, ub + 1, dtype=current_x.dtype, device=current_x.device
            )
        else:
            return (
                torch.randperm(
                    ub - lb + 1, dtype=current_x.dtype, device=current_x.device
                )
                + lb
            )[:max_num_cat_values]

    new_cat_values = torch.cat(
        tuple(
            _get_cat_values_tensor(bounds[0, dim], bounds[1, dim]) for dim in cat_dims
        ),
    )
