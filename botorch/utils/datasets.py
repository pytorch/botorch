...
class ContextualDataset(SupervisedDataset):
    ...
    def _extract_context_buckets(self) -> None:
            ...
            for outcome_name in self.outcome_names:
                for k, v in self.metric_decomposition.items():
                    if outcome_name in v:
                        self.context_buckets.append(k)
                        break
        else:
            self.context_buckets = list(self.parameter_decomposition.keys())
