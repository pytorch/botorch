...
try:
        ...
        def _evaluate(
            ...
            with torch.no_grad():
                with settings.cholesky_max_tries(9):
                    if isinstance(self.botorch_model, GenericDeterministicModel):
                        y = self.botorch_model.posterior(X=X).mean
                    else:
                        # eval in batch mode, since all we need is the mean
                        y = self.botorch_model.posterior(
                            X=X.unsqueeze(-2)
                        ).mean.squeeze(-2)
    ...
    def optimize_with_nsgaii(
        ...
        """Optimize the model's posterior mean using NSGA-II.
