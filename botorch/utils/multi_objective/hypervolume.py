...
def get_hypervolume_maximizing_subset(
    ...
    """Find the hypervolume-maximizing subset of size `n` from a set.
    ...
    for _ in range(n):
        ...
        for i in remaining_idcs:
            ...
            if hv > best_hv:
                ...
                best_hv = hv
        if best_idx is None:
            # no arm improved HV, so select a random arm. This will only happen if Y is not
            # a Pareto frontier, where all points are better than the reference point
