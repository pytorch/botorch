...
class BotorchTestCase(TestCase):
    ...
    def setUp(self, suppress_input_warnings: bool = True) -> None:
        ...
        Args:
            suppress_input_warnings: If True, suppress common input warnings (see below).
        """
...
class BaseTestProblemTestCaseMixIn:
    ...
    @classmethod
    @abstractmethod
    def functions(cls) -> Sequence[BaseTestProblem]:
...
class SyntheticTestFunctionTestCaseMixin:
    r"""Mixin for testing synthetic `BaseTestProblem`s aka test functions."""

    def test_optimal_value(self):
        r"""Test that a function's optimal_value is correctly computed, and defined if it should be."""
...
class MultiObjectiveTestProblemTestCaseMixin:
    ...
    This class provides test cases for attributes, maximum hypervolume, and reference points
    ...
    def test_ref_point(self):
        r"""Test the reference point (ref_point) attribute for each function (for each dtype)."""
        for dtype in (torch.float, torch.double):
            for f in self.functions:
...
class ConstrainedTestProblemTestCaseMixin:
    ...
    This class provides test cases for attributes and methods of constrained test problems,
    including testing the number of constraints and the evaluation of constraint slack.
    """
    ...
    def test_evaluate_slack(self):
        ...
        1. The evaluate_slack_true and evaluate_slack methods return tensors of the expected shape
        2. The relationship between evaluate_slack and evaluate_slack_true is consistent with
           the constraint_noise_std attribute of the function
        """
        for dtype in (torch.float, torch.double):
            for f in self.functions:
                f.to(device=self.device, dtype=dtype)
...
class TestCorruptedProblemsMixin(BotorchTestCase):
    ...
    This class provides setup and utility functions for testing corrupted test problems
    using a specified outlier generator and a Rosenbrock problem.
    """

    def setUp(self, suppress_input_warnings: bool = True) -> None:
        r"""Set up the test case with a dummy outlier generator and a Rosenbrock problem.
...
class MockPosterior(Posterior):
    r"""This class is used to simulate a posterior with specified mean, variance, and samples.
    ...
    def rsample(
        ...
        r"""Return mock samples by extending the shape of the initially specified samples.
...
def get_sampler_mock(
    ...
    Args:
        posterior: Used only for dispatching so that `get_sampler` works with a `MockPosterior`.
        sample_shape: The shape of the samples to generate.
