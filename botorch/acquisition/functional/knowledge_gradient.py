...
def knowledge_gradient(
    ...
    denom = (
        (torch.max(mean_noise, torch.zeros_like(mean_noise)) + covar_with_point[:, 0])
        .unsqueeze(1)
        .sqrt()
    )  # r x 1
    ...
    a = torch.stack(
        [torch.cat([mean[i : i + 1], tmp]) for i in range(X.shape[0])]
    )  # (1 + m)  # r x (1 + m)
...
def _filter_a_and_b(a, b):
    ...
    return torch.Tensor(
        [tmp[0] for tmp in filtered_b_a_pairs], [tmp[1] for tmp in filtered_b_a_pairs]
    )
