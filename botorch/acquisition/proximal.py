...
r"""
A wrapper around AquisitionFunctions to add proximal weighting of the
...
'proximal_weights'. Can only be used with acqusition functions based on single
batch models.
...
class ProximalAcquisitionFunction(AcquisitionFunction):
    ...
    def forward(self, X: Tensor) -> Tensor:
        ...
        diff = X - last_X
        L = torch.diag(torch.sqrt(self.proximal_weights)).reshape(1, d, d)
